...
/// The returned indices are in upstream order; passage ranking uses the same
/// chunk scores, so full-content selection does not require a second forward
/// pass.
fn select_documents(
    query: &str,
    raw: &[Raw],
